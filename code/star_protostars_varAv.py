...
import numpy as np
import argparse
parser = argparse.ArgumentParser(prog="star_protostars_varAv.py", description="Run Starfish fitting model in single order mode with many walkers, with extra black body component, and variable Av exponent")
...
from model_BB_varAv import ThetaParam, PhiParam
...
class Order(OrderBase):
    ...
    def update_Theta(self, p):
        ...
        A_lambda_Av =  (self.wl/6000.0)**p.exponentAv #std law is -1.74, Nishiyama is -2.0
...
def lnlike(p):
    ...
    try:
        pars1 = ThetaParam(grid=p[0:3], vz=p[3], vsini=p[4], logOmega=p[5], T_BB=p[6],
                            logOmega2=p[7], Av=p[8], exponentAv=p[9])
...
p0 = np.array(start["grid"] + [start["vz"], start["vsini"], start["logOmega"], start["T_BB"], 
              start["logOmega2"], start["Av"] , start["exponentAv"]] +
