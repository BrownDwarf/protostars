def user_defined_lnprior(p):
    ...
    if not ( (p[2] < 0.5) and (p[2] > 0) and
             ...
             (p[8] < 100.0) and (p[8] > 0.0) and
             (p[12] > 0.0) and (p[12] < 2.0) and
             (p[13] < -1.0) and (p[13] > -2.0) and
             (p[14] > 0.0) and (p[14] < 300 ) ):
    ...
    lnp_l = -1.0*(p[14] - 124.4)**2/(2.0*10.0**2)
    lnp_logAmp = -1.0*(p[13] + 1.40)**2/(2.0*0.03**2)
